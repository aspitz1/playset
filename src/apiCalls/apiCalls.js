...
const findCardsByName = async (name) => {
  ...
  try {
    const response = await fetch(
      `https://playset-api.onrender.com/api/proxy/cards/${formattedName}`
    );
    if (!response.ok) {
...
const findCardsById = async (magicApiId) => {
  try {
    const response = await fetch(
      `https://playset-api.onrender.com/api/proxy/card/${magicApiId}`
    );
    if (!response.ok) {
      const errMsg = await response.json();
      throw new Error(errMsg);
...
const deleteCardFromCollection = async (id) => {
  try {
    const response = await fetch(
      `https://playset-api.onrender.com/api/cards/${id}`,
      {
        method: "DELETE",
      }
    );
...
const updateCardInCollection = async (magicCard) => {
  try {
    const response = await fetch(
      `https://playset-api.onrender.com/api/cards/${magicCard.id}`,
      {
        method: "PUT",
        headers: {
          "Content-Type": "application/json",
